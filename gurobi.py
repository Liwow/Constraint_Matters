...
def solve_grb(filepath, log_dir, settings):
    ...
    for sn in range(solc):
        ...
        new_slacks.append(("obj_node", 1e20, None))  # objective node
...
def collect(ins_dir, q, sol_dir, log_dir, bg_dir, settings,solve=False):
    while True:
        filename = q.get()
        if not filename:
            break
        current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print(f"Start processing: {filename} at {current_time}")
        ...
        if not os.path.isfile(bg_file):
            if v_class_name is not None:
                A2, v_map2, v_nodes2, c_nodes2, b_vars2, v_class, c_class, _ = get_bigraph(filepath, False, v_class_name,
                                                                                           c_class_name)
            else:
                v_class = None
                c_class = None
...
if __name__ == '__main__':
    ...
    for size in sizes:
        ...
        for p in ps:
            p.close()  # close process resources
        gc.collect()
